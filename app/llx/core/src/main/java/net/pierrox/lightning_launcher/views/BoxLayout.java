...
public class BoxLayout extends ViewGroup implements SharedAsyncGraphicsDrawable.SharedAsyncGraphicsDrawableListener {
    ...
    private static final Paint sContentPaint;
    ...
    private final Runnable mUpdateAppWidgetHostViewSize = new Runnable() {
        @Override
        public void run() {
    ...
    public void pause() {
        if (mBoxBgNormal instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgNormal).pause();
        if (mBoxBgSelected instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgSelected).pause();
        if (mBoxBgFocused instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFocused).pause();
        if (mBoxBgFolder instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFolder).pause();
    }

    public void resume() {
        if (mBoxBgNormal instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgNormal).resume();
        if (mBoxBgSelected instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgSelected).resume();
        if (mBoxBgFocused instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFocused).resume();
        if (mBoxBgFolder instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFolder).resume();
    }

    private void registerDrawablesListeners() {
        if (mBoxBgNormal instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgNormal).registerListener(this);
        if (mBoxBgSelected instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgSelected).registerListener(this);
        if (mBoxBgFocused instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFocused).registerListener(this);
        if (mBoxBgFolder instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFolder).registerListener(this);
    }

    private void unregisterDrawablesListeners() {
        if (mBoxBgNormal instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgNormal).unregisterListener(this);
        if (mBoxBgSelected instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgSelected).unregisterListener(this);
        if (mBoxBgFocused instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFocused).unregisterListener(this);
        if (mBoxBgFolder instanceof SharedAsyncGraphicsDrawable)
            ((SharedAsyncGraphicsDrawable) mBoxBgFolder).unregisterListener(this);
    }
